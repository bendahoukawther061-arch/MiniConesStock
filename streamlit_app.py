...
for v in ventes:
pdf.cell(0, 8, f"Commande {v['num']} - {v['date']}", ln=True)
pdf.cell(0, 8, f"Client: {v['client']} | Revendeur: {v['revendeur']} | Chauffeur: {v['chauffeur']}", ln=True)
pdf.cell(0, 8, f"Charges: {v['charges']} DA | Total: {v['total']} DA | Bénéfice: {v['benefice']} DA", ln=True)
pdf.ln(5)
for p, info in v["produits"].items():
pdf.cell(0, 8, f"   {p}: {info['qte']} unité(s) x {info['prix_vente']} DA = {info['montant']} DA", ln=True)
pdf.ln(5)
buffer = BytesIO()
pdf.output(buffer)
buffer.seek(0)
...
if st.button("Enregistrer la commande"):
    vente = {
        "num": num, "date": date, "client": client, "revendeur": revendeur, "chauffeur": chauffeur,
...
for index, commande in enumerate(data["ventes"]):
st.markdown(f"### Commande {commande['num']} — {commande['date']}")
st.write(f"Client: {commande['client']} | Revendeur: {commande['revendeur']} | Chauffeur: {commande['chauffeur']}")
st.write(f"Charges: {commande['charges']} DA | Total: {commande['total']} DA | Bénéfice: {commande['benefice']} DA")
st.write("Produits :")
for p, info in commande["produits"].items():
st.write(f"- {p}: {info['qte']} unité(s) x {info['prix_vente']} DA = {info['montant']} DA")
col_mod, col_del = st.columns(2)
with col_mod:
if st.button("Modifier", key=f"mod_{index}"):
commande["client"] = st.text_input("Client", value=commande["client"], key=f"cli_{index}")
commande["revendeur"] = st.text_input("Revendeur", value=commande["revendeur"], key=f"rev_{index}")
commande["chauffeur"] = st.text_input("Chauffeur", value=commande["chauffeur"], key=f"chau_{index}")
save_data(data)
